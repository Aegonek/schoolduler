...
pub fn inverse_of_no_class_conflicts(chromosome: &Chromosome, decoder: &Decoder, leaderboard: &mut Leaderboard) -> Rating {
    ...
    let teacher_score = num::map_range(
        teacher_overlaps as f64
        , 0.0..=(leaderboard.max_teacher_overlaps as f64)
        , 1.0..=0.0
    );
    ...
    let group_score = num::map_range(
        group_overlaps as f64
        , 0.0..=(leaderboard.max_group_overlaps as f64)
        , 1.0..=0.0
    );

    // 6 first digits
    let rating = (teacher_score + group_score) / 2.0 * 100_000.0;
    rating as usize
}
...
fn teacher_overlaps(lessons: &[Class]) -> usize {
    ...
    let teachers = lessons.into_iter()
        .into_group_map_by(|cls| cls.teacher.clone());

    for teacher in teachers.values() {
        let hours = teacher.into_iter()
            .into_group_map_by(|cls| cls.lesson_hour);

        for duplicates in hours.values()
            .filter(|xs| xs.len() > 1) 
        {
...
fn group_overlaps(lessons: &[Class]) -> usize {
    ...
    let groups = lessons.into_iter()
        .into_group_map_by(|cls| cls.student_group.clone());

    for group in groups.values() {
        let hours = group.into_iter()
            .into_group_map_by(|cls| cls.lesson_hour);
