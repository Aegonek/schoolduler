...
pub fn write_by_student_group_sheet(schedule: &Schedule) -> Result<(), Box<dyn Error>> {
    let filename = time::timestamped("output/schedules_by_student_groups.xlsx")
        .to_str().expect("Expecting paths to contain only valid Unicode characters.")
        .to_owned();
    let workbook = Workbook::new(&filename);
    let mut schedule = schedule.clone();
    schedule.sort_by(|x, y| Ord::cmp(&x.student_group.year, &y.student_group.year).then
        (Ord::cmp(&x.student_group.suffix, &y.student_group.suffix)));

    let by_student_group = Itertools::group_by(schedule.into_iter(), |cls| cls.student_group.clone());
...
impl<'a> RowCursor<'a> {
    pub fn new(worksheet: &'a mut Worksheet<'a>) -> Self {
        RowCursor { worksheet, cur_row: 0, cur_col: 0, max_col: 0, pivot_col: 0 }
    }

    pub fn write_string(&mut self, text: &str) -> Result<(), XlsxError> {
        self.worksheet.write_string(self.cur_row, self.cur_col, text, None)?;
    ...
    pub fn write_number(&mut self, number: f64) -> Result<(), XlsxError> {
        self.worksheet.write_number(self.cur_row, self.cur_col, number, None)?;
        self.cur_col += 1;
        self.max_col = cmp::max(self.cur_col, self.max_col);
        Ok(())
    }
    ...
    pub fn write_datetime(&mut self, datetime: &xlsxwriter::DateTime) -> Result<(), XlsxError> {
        self.worksheet.write_datetime(self.cur_row, self.cur_col, datetime, None)?;
        self.cur_col += 1;
        self.max_col = cmp::max(self.cur_col, self.max_col);
        Ok(())
    }
